...
const iconMap: Record<string, React.ReactNode> = {
  ...
  alumni: <Users className="w-8 h-8" />,
  transport: <Users className="w-8 h-8" />,
  elearning: <BookOpen className="w-8 h-8" />,
  inventory: <BookOpen className="w-8 h-8" />,
  procurement: <BookOpen className="w-8 h-8" />,
};
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("Active school:", schoolRes.data);
        setSchoolId(schoolRes.data.id);

        const modulesRes = await api.get("/modules/");
        console.log("Raw modules from API:", modulesRes.data);
          ...
          {modules.map((module) => {
            ...
            return (
              ...
              >
                ...
                <h3 className={`text-lg font-bold mb-2 transition-colors ${isSelected ? "text-white" : "text-gray-900 group-hover:text-gray-800"}`}>
                  {module.title}
                </h3>
                <p className={`text-sm transition-colors ${isSelected ? "text-white/80" : "text-gray-600 group-hover:text-gray-700"}`}>
                  {module.description}
                </p>
              </motion.button>
            );
