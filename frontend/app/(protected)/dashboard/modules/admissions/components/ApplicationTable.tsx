...
// You will replace this with real auth check
// Examples:
// - const { user } = useAuth();
// - const isAdmin = user?.role === "ADMIN" || user?.is_superuser;
const isAdmin = true; // ← TEMPORARY — replace with your real check!
...
export default function ApplicationTable({
  ...
  const handleEnroll = async (id: string) => {
    try {
      ...
      toast.success("Student enrolled successfully!");
      router.push(`/dashboard/modules/students/${res.data?.id}`);
      // Optional: invalidate here too if needed
    ...
    <div className="w-full">
                ...
                <td className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-slate-400 mt-1 uppercase font-medium tracking-tighter truncate">
                        Grade:{" "}
                        {(typeof app.class_applied === "object"
                          ? app.class_applied?.name
                          : app.class_applied) || "Unassigned"}
                      </p>
                    </div>
                  </div>
                </td>
                ...
                <td className="p-4 text-right">
                  <div className="flex justify-end gap-2">
                    ...
                    <ActionMenu
                      ...
                      isAdmin={isAdmin}
                      loadingEnroll={enrollingId === app.id}
                      loadingDelete={deletingId === app.id}
                    />
                  </div>
                </td>
              </tr>
      ...
      <div className="md:hidden space-y-3 px-1">
        {filtered.map((app) => (
          <div
            key={app.id}
            className="bg-white p-4 rounded-[1.5rem] border border-slate-200 shadow-sm overflow-hidden"
          >
            <div className="flex items-start justify-between gap-3 mb-4">
              <div className="flex items-center gap-3 min-w-0 flex-1">
                <div className="h-10 w-10 shrink-0 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-base shadow-md">
            ...
            <div className="flex items-center gap-2">
              <Link href={`/dashboard/modules/admissions/${app.id}`} className="flex-1">
                <Button
                  variant="outline"
                  className="w-full h-10 rounded-xl font-bold text-[10px] uppercase tracking-wider border-slate-200"
                >
                  View Dossier
                </Button>
              </Link>
...
function ActionMenu({
  ...
  const canDelete = isAdmin && app.status === "DRAFT";
        ...
        {canDelete && (
          <>
            ...
            <DropdownMenuItem
              ...
              className="rounded-xl text-red-600 focus:bg-red-50 focus:text-red-700 font-medium cursor-pointer text-sm"
            >
              <Trash2 className="h-4 w-4 mr-2" />
              {loadingDelete ? "Deleting..." : "Delete Draft"}
            </DropdownMenuItem>
          </>
        )}
      </DropdownMenuContent>
